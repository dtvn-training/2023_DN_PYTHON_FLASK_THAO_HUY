...
class Creatives(db.Model):
    creative_id = db.Column(db.NVARCHAR(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    ...
    campaign_id = db.Column(db.NVARCHAR(36), db.ForeignKey('campaigns.campaign_id'), nullable=False, default=uuid.uuid4().bytes)
